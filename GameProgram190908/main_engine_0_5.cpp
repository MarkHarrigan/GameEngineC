...
char outputvsn[] = "Version 0.3";
char output;

// clear the back buffer out
// linear memory?
//if (ddsd.lPitch == GetSystemMetrics(SM_CXSCREEN))
//   memset(back_buffer,0,screen_x*screen_y);
//else
//{
//	// non-linear memory
//
//	// make a copy of the video pointer
//	UCHAR *dest_ptr = back_buffer;
//
//	// clear out the memory one line at a time
//	for (int y=0; y < screen_y; y++)
//	{
//		// clear the next line
//		memset(dest_ptr,0,screen_x);
//
...
dest_rect.left = dest_rect.left + velocity_x;
dest_rect.top = dest_rect.top + velocity_y;
dest_rect.right = dest_rect.right + velocity_x;
dest_rect.bottom = dest_rect.bottom + velocity_y;
...
if (FAILED(lpddsprimary->Blt(NULL, // pointer to dest RECT
							  NULL, // pointer to source surface
							  NULL, // pointer to source RECT
							  DDBLT_COLORFILL | DDBLT_WAIT, // do a colour fill and wait if you have to
							  &ddbltfx))) // pointer the DDBLTFX holding info
...
//TextOut(hdc,10,10,outputtitle,strlen(outputtitle));
//TextOut(hdc,10, 25,outputvsn,strlen(outputvsn));
//TextOut(hdc,10, 40,xPosStr,strlen(xPosStr));
//TextOut(hdc,10, 55,yPosStr,strlen(yPosStr));

//************* Version 0.3 uncommented *********************
...
//if (FAILED(lpdd->SetDisplayMode(GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), SCREEN_BPP,0,0)))
